...
const (
	...
	EdgeFrontingDialOverrides                           = "EdgeFrontingDialOverrides"
	EdgeFrontingDialOverridesProbability                = "EdgeFrontingDialOverridesProbability"
	EdgeFrontingScanSpecParameter                    = "EdgeFrontingScanSpec"
	EdgeFrontingScanUseBuiltInSpec                   = "EdgeFrontingScanUseBuiltInSpec"
...
var defaultParameters = map[string]struct {
	...
	EdgeFrontingDialOverrides:                     {value: EdgeFrontingDialOverrideSpecs{}},
	EdgeFrontingDialOverridesProbability:          {value: 1.0, minimum: 0.0},
	EdgeFrontingScanSpecParameter:              {value: EdgeFrontingScanSpec{}},
	EdgeFrontingScanUseBuiltInSpec:             {value: false},
...
func (p *Parameters) Set(
	...
	for i := 0; i < len(applyParameters); i++ {
		...
		for name, value := range applyParameters[i] {
			...
			case EdgeFrontingDialOverrideSpecs:
				err := v.Validate(customTLSProfileNames)
				if err != nil {
					if skipOnError {
						continue
...
// EdgeFrontingDialOverrides returns a EdgeFrontingDialOverrideSpecs parameter value.
func (p ParametersAccessor) EdgeFrontingDialOverrides(name string) EdgeFrontingDialOverrideSpecs {
	...
	if ok {
		...
		if !prng.FlipWeightedCoin(probabilityValue) {
			defaultParameter, ok := defaultParameters[name]
			if ok {
				defaultValue, ok := defaultParameter.value.(EdgeFrontingDialOverrideSpecs)
				if ok {
					value := make(EdgeFrontingDialOverrideSpecs, len(defaultValue))
					copy(value, defaultValue)
					return value
				}
			}
	...
	value := EdgeFrontingDialOverrideSpecs{}
	p.snapshot.getValue(name, &value)
	return value
}

// EdgeFrontingScanSpec returns a EdgeFrontingScanSpec parameter value.
func (p ParametersAccessor) EdgeFrontingScanSpec(name string) EdgeFrontingScanSpec {
	value := EdgeFrontingScanSpec{}
	p.snapshot.getValue(name, &value)
	return value
}
