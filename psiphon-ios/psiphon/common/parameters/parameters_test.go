...
func TestGetDefaultParameters(t *testing.T) {
	...
	for name, defaults := range defaultParameters {
		...
		case EdgeFrontingDialOverrideSpecs:
			g := p.Get().EdgeFrontingDialOverrides(name)
			if !reflect.DeepEqual(v, g) {
				t.Fatalf("EdgeFrontingDialOverrideSpecs returned %+v expected %+v", g, v)
			}
		case EdgeFrontingScanSpec:
			g := p.Get().EdgeFrontingScanSpec(name)
			if !reflect.DeepEqual(v, g) {
				t.Fatalf("EdgeFrontingScanSpec returned %+v expected %+v", g, v)
			}
