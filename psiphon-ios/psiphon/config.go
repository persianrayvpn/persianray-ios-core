...
type Config struct {
	...
	// PersianRay SOCKS/HTTP policy (ads / adult / Safe Search / local bypass).
	// Applied in DialPersianRay before tunnel Dial. Not sent to Psiphon servers.
	// Host lists come from the iOS app (PolicyDomains.swift) so a release can
	// update domains without rebuilding this xcframework. Empty slices fall
	// back to persianray_policy_lists.go.
	PersianRayBlockAds         bool     `json:",omitempty"`
	PersianRayBlockAdult       bool     `json:",omitempty"`
	PersianRaySafeSearch       bool     `json:",omitempty"`
	PersianRayBypassLocal      bool     `json:",omitempty"`
	PersianRayAdExact          []string `json:",omitempty"`
	PersianRayAdSuffix         []string `json:",omitempty"`
	PersianRayAdultExact       []string `json:",omitempty"`
	PersianRayAdultSuffix      []string `json:",omitempty"`
	PersianRayLocalSuffix      []string `json:",omitempty"`
	PersianRaySafeSearchExact  []string `json:",omitempty"`
	PersianRayDoHExact         []string `json:",omitempty"`
	PersianRayDoHSuffix        []string `json:",omitempty"`
	...
	// EdgeFrontingDialOverrides configures optional fronted meek outer-dial
	// replacement rules.
	EdgeFrontingDialOverrides            parameters.EdgeFrontingDialOverrideSpecs `json:",omitempty"`
	EdgeFrontingDialOverridesProbability *float64                                `json:",omitempty"`
	EdgeFrontingScanSpec              parameters.EdgeFrontingScanSpec       `json:",omitempty"`
	EdgeFrontingScanUseBuiltInSpec    bool                                    `json:",omitempty"`
...
func (config *Config) EdgeFrontingEnabled() bool {
	if config == nil {
		return false
	}
	...
	return len(params.Get().EdgeFrontingDialOverrides(
		parameters.EdgeFrontingDialOverrides)) > 0 ||
		params.Get().Bool(parameters.EdgeFrontingScanUseBuiltInSpec) ||
		!params.Get().EdgeFrontingScanSpec(
			parameters.EdgeFrontingScanSpecParameter).IsEmpty()
}
...
func (config *Config) makeConfigParameters() map[string]interface{} {
	...
	if len(config.EdgeFrontingDialOverrides) > 0 {
		applyParameters[parameters.EdgeFrontingDialOverrides] = config.EdgeFrontingDialOverrides
	}

	if config.EdgeFrontingDialOverridesProbability != nil {
		applyParameters[parameters.EdgeFrontingDialOverridesProbability] = *config.EdgeFrontingDialOverridesProbability
	}

	if !config.EdgeFrontingScanSpec.IsEmpty() {
		applyParameters[parameters.EdgeFrontingScanSpecParameter] = config.EdgeFrontingScanSpec
	}

	if config.EdgeFrontingScanUseBuiltInSpec {
		applyParameters[parameters.EdgeFrontingScanUseBuiltInSpec] = true
	}
...
func (config *Config) setDialParametersHash() {
	...
	if len(config.EdgeFrontingDialOverrides) > 0 {
		hash.Write([]byte("EdgeFrontingDialOverrides"))
		for _, override := range config.EdgeFrontingDialOverrides {
	...
	if config.EdgeFrontingDialOverridesProbability != nil {
		hash.Write([]byte("EdgeFrontingDialOverridesProbability"))
		binary.Write(hash, binary.LittleEndian, *config.EdgeFrontingDialOverridesProbability)
	}

	if !config.EdgeFrontingScanSpec.IsEmpty() {
		hash.Write([]byte("EdgeFrontingScanSpec"))
		encodedSpec, _ := json.Marshal(config.EdgeFrontingScanSpec)
		hash.Write(encodedSpec)
	}

	if config.EdgeFrontingScanUseBuiltInSpec {
		hash.Write([]byte("EdgeFrontingScanUseBuiltInSpec"))
	}
