...
func NewHttpProxy(
	...
	tunneledDialer := func(_, addr string) (conn net.Conn, err error) {
		return DialPersianRay(config, tunneler, addr, nil)
	}
...
func (proxy *HttpProxy) httpConnectHandler(localConn net.Conn, target string) (err error) {
	...
	remoteConn, err := DialPersianRay(proxy.config, proxy.tunneler, target, localConn)
	if err != nil {
...
func (proxy *HttpProxy) makeRewriteICYClient() (*http.Client, *rewriteICYStatus) {
	...
	tunneledDialer := func(_, addr string) (conn net.Conn, err error) {
		...
		return DialPersianRay(proxy.config, proxy.tunneler, addr, nil)
	}
