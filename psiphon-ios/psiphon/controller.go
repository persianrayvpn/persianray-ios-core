...
type Controller struct {
	...
	edgeFrontingScanRouteFound            int32
	...
	serverEntryIterationEdgeFrontingCandidateCount    int
...
loop:
	for {
		...
		case connectedTunnel := <-controller.connectedTunnels:
			...
			noticeEdgeFrontingScanConnected(connectedTunnel.dialParams)
...
func (p *protocolSelectionConstraints) conditionallyEnabledComponents() conditionallyEnabledComponents {
	return conditionallyEnabledComponents{
		edgeFrontingEnabled: p.config.EdgeFrontingEnabled(),
	}
}
...
func (controller *Controller) isEdgeFrontingScanRouteFound() bool {
	return atomic.LoadInt32(&controller.edgeFrontingScanRouteFound) != 0
}

func (controller *Controller) markEdgeFrontingScanRouteFound() {
	atomic.StoreInt32(&controller.edgeFrontingScanRouteFound, 1)
}
...
func (controller *Controller) startEstablishing() {
	...
	controller.concurrentEstablishTunnelsMutex.Unlock()
	atomic.StoreInt32(&controller.edgeFrontingScanRouteFound, 0)
...
func (controller *Controller) resetServerEntryIterationMetrics() {
	...
	controller.serverEntryIterationEdgeFrontingCandidateCount = 0
		...
		for {
			...
			if controller.config.AggressiveEstablishment && !wasServerAffinityCandidate {
				...
				for _, tunnelProtocol := range supportedProtocols {
					isEdgeFrontingScanProtocol :=
						protocol.TunnelProtocolUsesFrontedMeekEdge(tunnelProtocol)
					if isEdgeFrontingScanProtocol &&
						controller.isEdgeFrontingScanRouteFound() {
						continue
					}
					fanout := 1
					if isEdgeFrontingScanProtocol {
						p := controller.config.GetParameters().Get()
						fanout = edgeFrontingScanCandidateFanout(
							p,
							controller.config.AggressiveEstablishment)
						p.Close()
					}
...
loop:
	for candidateServerEntry := range controller.candidateServerEntries {
		...
		if candidateServerEntry.preSelectedProtocol != "" &&
			protocol.TunnelProtocolUsesFrontedMeekEdge(candidateServerEntry.preSelectedProtocol) &&
			controller.isEdgeFrontingScanRouteFound() {
			continue
		}
		...
		dialParams, err := MakeDialParameters(
			...
			controller.serverEntryIterationEdgeFrontingCandidateCount)
		...
		if dialParams.EdgeFrontingScanCandidate &&
			controller.isEdgeFrontingScanRouteFound() {
			controller.concurrentEstablishTunnelsMutex.Unlock()
		...
		if protocol.TunnelProtocolUsesFrontedMeekEdge(dialParams.TunnelProtocol) {
			controller.serverEntryIterationEdgeFrontingCandidateCount += 1
		}
		...
		if err != nil {
			...
			recordEdgeFrontingScanResult(dialParams, false)
		...
		if isEdgeFrontingScanDialParams(dialParams) {
			controller.markEdgeFrontingScanRouteFound()
		}
		recordEdgeFrontingScanResult(dialParams, true)
