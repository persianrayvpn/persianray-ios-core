...
func TestFrontedMeekQUICCDNDialParametersAndReplay(t *testing.T) {
	runDialParametersAndReplay(t, protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_EDGE_OSSH)
}
...
func runDialParametersAndReplay(t *testing.T, tunnelProtocol string) {
	...
	if protocol.TunnelProtocolUsesFrontedMeekEdge(tunnelProtocol) {
		applyParameters[parameters.EdgeFrontingDialOverrides] = parameters.EdgeFrontingDialOverrideSpecs{
		...
		applyParameters[parameters.EdgeFrontingDialOverridesProbability] = 1.0
	}
	...
	if tunnelProtocol == protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_EDGE_OSSH {
...
func TestEdgeFrontingDialOverrideCandidateNumber(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			dialParams := &DialParameters{
				EdgeFrontingCandidateNumber: testCase.cdnCandidateNumber,
			}

			candidateNumber := dialParams.edgeFrontingDialOverrideCandidateNumber()
...
func TestEdgeFrontingDialOverrideProtocolConstraints(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			tunnelProtocol:            protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_EDGE,
		...
		},
		{
			...
			tunnelProtocol:            protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_EDGE_OSSH,
			inputSNI:                  "cdn.example.org",
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			dialParams.applyEdgeFrontingDialOverrideProtocolConstraints(testCase.originalSNI)
