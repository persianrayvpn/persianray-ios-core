...
type DialParameters struct {
	...
	EdgeFrontingCandidateNumber                 int  `json:"-"`
	EdgeFrontingScanCandidate                   bool `json:"-"`
	edgeFrontingScanStateKey                    string
...
func MakeDialParameters(
	...
	edgeFrontingCandidateNumbers ...int) (*DialParameters, error) {
	...
	edgeFrontingCandidateNumber := 0
	if len(edgeFrontingCandidateNumbers) > 0 {
		edgeFrontingCandidateNumber = edgeFrontingCandidateNumbers[0]
	}
	...
	dialParams.EdgeFrontingCandidateNumber = edgeFrontingCandidateNumber
	...
	case protocol.TUNNEL_PROTOCOL_FRONTED_MEEK,
		protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_EDGE,
		protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_OBFUSCATED_SSH,
		protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_EDGE_OSSH:
	...
	case protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_HTTP,
		protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_HTTP_EDGE:
	...
	if (!isReplay || !replayFronting) &&
		...
		var ok bool
		ok, err = dialParams.applyEdgeFrontingDialOverride(p)
		if err != nil {
			return nil, errors.Trace(err)
		}
		if !ok {
...
func (dialParams *DialParameters) applyEdgeFrontingDialOverride(
	p parameters.ParametersAccessor) (bool, error) {

	override, scanCandidate, scanStateKey, ok, err := selectEdgeFrontingScanOverride(
		p,
		dialParams.NetworkID,
		dialParams.FrontingProviderID,
		dialParams.MeekFrontingDialAddress,
		dialParams.MeekFrontingHost,
		dialParams.edgeFrontingDialOverrideCandidateNumber())
	if err != nil {
	...
	if !ok {
		...
		dialParams.EdgeFrontingScanCandidate = true
		dialParams.edgeFrontingScanStateKey = scanStateKey
		dialParams.edgeFrontingScanSelectedCandidate = *scanCandidate
	}
	...
	if override.TLSProfile != "" &&
		...
		err = dialParams.applyEdgeFrontingDialOverrideTLSProfile(
	...
	dialParams.applyEdgeFrontingDialOverrideProtocolConstraints(originalMeekSNIServerName)

	return true, nil
}

func (dialParams *DialParameters) applyEdgeFrontingDialOverrideProtocolConstraints(
	originalMeekSNIServerName string) {

	switch protocol.TunnelProtocolMinusInproxy(dialParams.TunnelProtocol) {
	case protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_HTTP_EDGE:
	...
	case protocol.TUNNEL_PROTOCOL_FRONTED_MEEK_QUIC_EDGE_OSSH:
...
func (dialParams *DialParameters) edgeFrontingDialOverrideCandidateNumber() int {

	if dialParams.EdgeFrontingCandidateNumber < 0 {
		return 0
	}

	return dialParams.EdgeFrontingCandidateNumber
}

func (dialParams *DialParameters) applyEdgeFrontingDialOverrideTLSProfile(
