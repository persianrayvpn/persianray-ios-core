...
type MaintainTunnelConfig struct {
	...
	// OnReady is called after a successful CONNECT-IP handshake. It must not block.
	OnReady func()
	// OnError receives connection and pump failures. It must not block.
	OnError func(error)
}
...
func MaintainTunnel(ctx context.Context, cfg MaintainTunnelConfig) {
	if cfg.UseHTTP2 {
		if _, ok := cfg.Endpoint.(*net.TCPAddr); !ok {
			if cfg.OnError != nil {
				cfg.OnError(fmt.Errorf("HTTP/2 mode requires a *net.TCPAddr endpoint, got %T", cfg.Endpoint))
			}
			return
		}
	} else {
		if _, ok := cfg.Endpoint.(*net.UDPAddr); !ok {
			if cfg.OnError != nil {
				cfg.OnError(fmt.Errorf("HTTP/3 mode requires a *net.UDPAddr endpoint, got %T", cfg.Endpoint))
			}
			return
		}
	}
	...
	for {
		...
		if rsp.StatusCode != 200 {
			...
			if cfg.OnError != nil {
				cfg.OnError(fmt.Errorf("tunnel connection failed: %s", rsp.Status))
			}
			_ = ipConn.Close()
			if tr != nil {
				_ = tr.Close()
